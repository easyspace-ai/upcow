...
尾盘风险来自“最后几十秒反转/跳变”。策略必须分段降风险：

- **T <= 90s**：进入 `reduce-only`（不再扩大净敞口，只允许让 \(|\Delta_{inv}|\) 变小）
- **T <= 30s**：只允许“快速回到 Delta=0”（必要时用 FAK），不再做做市
- **T <= 10s**：尽量清空未成交挂单，避免最后秒被动成交留下单边

...
- **交易窗口（简化版）**：
  - **不做盘前**（开盘前不交易）
  - **盘中只做前 12 分钟**：开盘后到第 12 分钟（含）允许做市/交易
  - **最后 3 分钟尽量不交易**：第 12–15 分钟进入“风控窗口”，以**撤单+降敞口**为主，避免尾部跳变风险
...
## 5. 尾盘风控（最重要的硬规则：最后 3 分钟尽量不交易）

尾盘风险来自“最后几十秒反转/跳变”。为了降低复杂度，本策略采用**硬时间窗**：

- **开盘后 12:00–15:00**：进入 `risk-only`（尽量不交易）
  - 立即撤掉所有可能“扩大风险”的挂单（通常是四边做市单）
  - 不再新增净敞口（禁止让 \(|\Delta_{inv}|\) 变大）
  - 如果 \(|\Delta_{inv}|\) 仍然较大：允许用小单/分步将 \(\Delta_{inv}\rightarrow 0\)（必要时用 FAK），目标是在**第 12 分钟左右把风险解决掉**
...
- **尾盘**
  - `tradeStopAtSeconds`: 12*60（12 分钟后停止常规交易/做市）
  - `forceFlattenAtSeconds`: 12*60（进入 risk-only 后优先把 \(\Delta_{inv}\) 压回 0）

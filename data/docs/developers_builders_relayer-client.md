...
Your system creates transactions on behalf of users


Your system uses Builder API credentials locally to add headers


Complete signed request is sent directly to the relayer
...
Used for:
 Magic Link users from Polymarket.com


Deployment:
 Automatic on first transaction
...
The 
execute
 method sends transactions through the relayer. Pass an array of transactions to batch multiple operations in a single call.
...
:
 string
;  
// Encoded function call (use "0x" for simple transfers)
...
:
 string
; 
// Amount of MATIC to send (usually "0")
...
STATE_INVALID
Transaction rejected as invalid (terminal ❌)

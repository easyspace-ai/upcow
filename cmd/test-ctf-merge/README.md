...
这个 demo 演示了如何使用 CTF (Conditional Token Framework) 进行 YES + NO 代币的合并操作。

## 功能说明

1. **自动获取市场**: 自动获取当前 BTC 15 分钟市场的信息
2. **Merge 操作**: 将 YES + NO 代币合并回 USDC（1 YES + 1 NO -> 1 USDC）

## 前置要求

1. 在当前目录（`cmd/test-ctf-merge/`）创建 `.env` 配置文件，包含以下内容：
```bash
# 必需配置
PRIVATE_KEY=0x你的私钥
...
2. 确保账户有足够的：
   - **YES 和 NO 代币余额**（用于 merge 操作）
     - Relayer 模式：代理地址需要有代币余额
     - 直接调用模式：交易账户地址需要有代币余额
...
```bash
# 进入目录
cd cmd/test-ctf-merge

# 创建 .env 文件（参考上面的格式）
# 然后运行（使用默认参数，合并 1.0 USDC 等值的代币）
go run main.go
```
...
- `AMOUNT`: 可选，要合并的 USDC 数量（默认 1.0，实际合并数量取 YES 和 NO 余额的最小值）
...
2. **检查余额**: 显示 USDC 余额以及 YES/NO 代币余额
3. **Merge 操作**:
   - 检查 YES/NO 代币余额
   - 创建合并交易（数量取两者最小值）
   - 显示交易详情
...
- ⚠️ **余额要求**：交易账户地址需要有足够的 YES 和 NO 代币
...
2. **余额检查**: Merge 操作需要同时拥有足够的 YES 和 NO 代币。实际合并数量取两者最小值
3. **市场可用性**: 如果当前周期的市场尚未创建，程序会报错。请稍后再试
4. **Builder API 凭证**: 如果使用 Relayer 模式，需要从 Polymarket Builder Program 获取 API 凭证
5. **市场选择**: 本工具会自动获取**当前** BTC 15 分钟市场，用于合并当期代币

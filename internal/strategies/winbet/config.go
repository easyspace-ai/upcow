...
//
// 核心策略逻辑：
// - 通过监控 UP/DOWN 价格变化速度来决定是否开单
// - 目标：在一个周期中尽可能多地开对冲单，每一对对冲单锁定一定的利润
// - 当周期交易上千次时，小利润累积变得可观
//
// 交易流程：
// - 当某一侧速度超过阈值时，触发交易：
...
type Config struct {
	// ====== UI 配置 ======
	EnableUI            bool `yaml:"enableUI" json:"enableUI"`                       // 是否启用 UI，默认 false
	UIRefreshIntervalMs int  `yaml:"uiRefreshIntervalMs" json:"uiRefreshIntervalMs"` // UI 刷新间隔（毫秒），默认 500
	...
	HedgeTimeoutFakSeconds     int `yaml:"hedgeTimeoutFakSeconds" json:"hedgeTimeoutFakSeconds"`         // 对冲单超时后以 FAK 吃单的时间（秒），0=禁用，默认 0
...
func (c *Config) Validate() error {
	...
	// UI 配置默认值
	if c.UIRefreshIntervalMs <= 0 {
		c.UIRefreshIntervalMs = 500 // 默认 500ms
	}
	...
	// hedgeTimeoutFakSeconds: 0 表示禁用，>0 表示从 Entry 成交开始，超时后撤单并以卖一价吃单（FAK）

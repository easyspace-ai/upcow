    ...
    with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if '📝' in msg and '纸交易' in msg and '模拟下单' in msg:
                ...
                # SELL订单视为出场订单
                if order_info.get('side') == 'SELL':
                    exit_info = {
                        'timestamp': parsed['timestamp'],
                        'exit_price': int(order_info.get('price', 0) * 100) if order_info.get('price') else 0,
                        'size': order_info.get('size', 0),
                        'reason': 'sell_order',
                        'order_id': order_info.get('order_id', ''),
                        'type': 'exit'
                    }
                    # 从assetID判断token类型（需要从订单簿或交易记录中推断）
            ...
            # 出场记录（从"出场"日志中提取）
            if '📤' in msg and '出场' in msg:
                exit_info = {}
                exit_info['timestamp'] = parsed['timestamp']
                
                # 提取token
                token_match = re.search(r'token=(\w+)', msg)
                if token_match:
                    exit_info['token'] = token_match.group(1).lower()
                
                # 提取价格
                bid_match = re.search(r'bid=(\d+)c', msg)
                if bid_match:
                    exit_info['exit_price'] = int(bid_match.group(1))
                
                # 提取数量
                size_match = re.search(r'size=([\d.]+)', msg)
                if size_match:
                    exit_info['size'] = float(size_match.group(1))
                
                # 提取原因
                reason_match = re.search(r'reason=(\w+)', msg)
                if reason_match:
                    exit_info['reason'] = reason_match.group(1)
                
                exit_info['type'] = 'exit_log'
                exits.append(exit_info)
    
    return orders, trades, exits
...
def match_exits_to_trades(matched_trades, exits, orders):
    """将出场订单匹配到交易"""
    # 为每个交易找到对应的SELL订单
...
def print_profit_analysis(matched_trades):
    """打印利润分析"""
    print("=" * 120)
    print("📊 交易订单和利润分析")
    print("=" * 120)
    ...
    # 表格标题
    print(f"{'交易#':<6} {'时间':<20} {'方向':<6} {'Entry价格':<12} {'Hedge价格':<12} {'Entry状态':<12} {'Hedge状态':<12} {'出场价格':<12} {'Entry成本':<12} {'Hedge成本':<12} {'总成本':<12} {'出场收入':<12} {'利润(USDC)':<14} {'利润(c)':<12} {'状态':<12}")
    print("-" * 120)
    ...
    for i, trade_data in enumerate(matched_trades, 1):
        trade = trade_data['trade']
        entry_order = trade_data['entry_order']
        hedge_order = trade_data['hedge_order']
        exit_orders = trade_data['exit_orders']
        ...
        print(f"{i:<6} {time_str:<20} {side:<6} {entry_price:<12} {hedge_price:<12} {entry_status:<12} {hedge_status:<12} {exit_price_str:<12} {entry_cost:<12} {hedge_cost:<12} {total_cost:<12} {exit_revenue:<12} {profit_usdc:<14} {profit_cents:<12} {status:<12}")
    ...
    print(f"{'总计':<6} {'':<20} {'':<6} {'':<12} {'':<12} {'':<12} {'':<12} {'':<12} {total_entry_cost:<12.4f} {total_hedge_cost:<12.4f} {total_entry_cost + total_hedge_cost:<12.4f} {total_exit_revenue:<12.4f} {total_profit:<14.4f} {total_profit_cents:<12.2f} {'':<12}")
    print()
    ...
    for i, trade_data in enumerate(matched_trades, 1):
        ...
        print(f"  方向: {trade.get('side', 'N/A').upper()}")
        print(f"  Entry价格: {trade.get('entry_price', 0)}c")
        print(f"  Hedge价格: {trade.get('hedge_price', 0)}c")
        
        if entry_order:
            print(f"  Entry订单: {entry_order.get('order_id', 'N/A')[:20]}... @ {entry_order.get('price', 0):.4f} size={entry_order.get('size', 0):.4f} status={entry_order.get('status', 'N/A')}")
        else:
            print(f"  Entry订单: 未找到")
        
        if hedge_order:
            print(f"  Hedge订单: {hedge_order.get('order_id', 'N/A')[:20]}... @ {hedge_order.get('price', 0):.4f} size={hedge_order.get('size', 0):.4f} status={hedge_order.get('status', 'N/A')}")
        else:
            print(f"  Hedge订单: 未找到")
        ...
        if exit_orders:
            ...
            print(f"  出场订单: {len(unique_exits)} 笔")
            for j, exit_order in enumerate(unique_exits.values(), 1):
                print(f"    {j}. {exit_order.get('token', 'N/A').upper()} @ {exit_order.get('exit_price', 0)}c size={exit_order.get('size', 0):.4f} reason={exit_order.get('reason', 'N/A')}")
        else:
            print(f"  出场订单: 未出场")
        
        print(f"  利润分析:")
        print(f"    Entry成本: {profit_info['entry_cost']:.4f} USDC")
        print(f"    Hedge成本: {profit_info['hedge_cost']:.4f} USDC")
        print(f"    总成本: {profit_info['total_cost']:.4f} USDC")
        print(f"    出场收入: {profit_info['exit_revenue']:.4f} USDC")
        print(f"    利润: {profit_info['profit']:.4f} USDC ({profit_info['profit_cents']:.2f} cents)")
        print(f"    状态: {profit_info['status']}")
        print()
...
if __name__ == "__main__":
    ...
    orders, trades, exits = analyze_profit(latest_log)
    matched_trades = match_orders_to_trades(orders, trades)
    matched_trades = match_exits_to_trades(matched_trades, exits, orders)
